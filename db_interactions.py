...
def import_from_stock_info_data(stock_ticker=""):
    """
...
def export_to_stock_info_data(stock_price_data_df=""):
    """
    ...
    try:
        stock_price_data_df.to_sql(name="stock_info_data", con=db_con, index=False, if_exists="append")
...
def import_from_stock_price_data(amount = 1):
    ...
    - KeyError: If the secrets cannot be fetched.
    - KeyError: If the connection to the database cannot be established.
    - KeyError: If the stock_price_data cannot be fetched from stock_price_data in the database to stock_price_data_df.    
    """
    if amount < 1:
...
def import_stock_financial_data(amount = 1, stock_ticker=""):
    ...
    - KeyError: If the connection to the database cannot be established.
    - 
    - KeyError: If the stock_financial_data_df cannot be created through the imported data from stock_income_stmt_data, stock_balancesheet_data and stock_cash_flow_data.
    - KeyError: If the stock_financial_data_df cannot be created through the imported data from stock_income_stmt_data, stock_balancesheet_data and stock_cash_flow_data with a specific ticker.
    """
    try:
        # Fetch the secrets from the secret_import function
        db_host, db_user, db_pass, db_name = fetch_secrets.secret_import()
    ...
    try:
        if stock_ticker == "":
            income_stmt_quary = f"""SELECT * FROM
                (SELECT * FROM stock_income_stmt_data ORDER BY date DESC LIMIT {amount}) AS temp
                ORDER BY date ASC
                """
            balancesheet_quary = f"""SELECT * FROM
                (SELECT * FROM stock_balancesheet_data ORDER BY date DESC LIMIT {amount}) AS temp
                ORDER BY date ASC
                """
            cash_flow_quary = f"""SELECT * FROM
                (SELECT * FROM stock_cash_flow_data ORDER BY date DESC LIMIT {amount}) AS temp
                ORDER BY date ASC
                """
            ...
            stock_financial_data_df = pd.merge(stock_income_stmt_data_df, stock_balancesheet_data_df, on=["date", "ticker"])
            stock_financial_data_df = pd.merge(stock_financial_data_df, stock_cash_flow_data_df, on=["date", "ticker"])
...
def export_to_stock_financial_data(stock_financial_data_df=""):
    """
